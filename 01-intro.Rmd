...
jobs %>% 
  spread(symbol,price) %>%
  mutate(ICSA = ICSA/1000) -> jobs

jobs %>% ggplot(aes(x=date,y=ICSA,label=ICSA)) + 
  ...
  labs(x = "", y = "", color = "Series", 
    ...
    coord_cartesian(xlim=c(startdate,enddate),ylim=c(floor(min(jobs$ICSA)),ceiling(max(jobs$ICSA))))
```
...
knitr::kable(
  tail(jobs2, 20), caption = "Initial Unemployment Claims for Most Recent 20 Weeks \n (1000's)", booktabs = TRUE, 
