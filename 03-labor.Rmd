...
jobs %>% 
  spread(symbol,price) -> jobs

ggplot(data = jobs, aes(x=date,y=CIVPART,label=CIVPART)) + 
  geom_rect(data=recessions.df, inherit.aes=F, aes(xmin=Peak, xmax=Trough, ymin=-Inf, ymax=+Inf), fill='darkgray', alpha=0.5) +
  geom_line(size=1.05) + 
  theme_light() +
  geom_point(data=filter(jobs,date==max(jobs$date)),size=2,alpha=0.75)+
  geom_text(data=filter(jobs,date==max(jobs$date)),fontface="bold",size=4,nudge_y=-.25)+
  ...
  labs(x="",y="",title="Labor Force Participation Rate",
       ...
       caption="Original code by Len Kiefer, adapted by Andre R. Neveu. Source: U.S. Bureau of Labor Statistics.\n Shaded bars NBER Recessions Civilian Labor Force Participation Rate, May 25, 2020.")+
    coord_cartesian(xlim=c(startdate,enddate),ylim=c(floor(min(jobs$CIVPART)),ceiling(max(jobs$CIVPART))))
...
```{r neat6, echo=FALSE, warning=FALSE, fig.cap='Labor Force Participation Has Plummetted', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(data=dfp, aes(x=date,y=v,color=var, label=var))+
  geom_rect(data=recessions.df, inherit.aes=F, aes(xmin=Peak, xmax=Trough, ymin=-Inf, ymax=+Inf), fill='darkgray', alpha=0.5) +
  geom_line(size=1.05)+theme_minimal()+
  geom_point(data=filter(dfp,date==max(dfp$date)),size=2,alpha=0.75)+
